...
### Must Complete
- [ ] #37: Align config format with existing CAMI workspace (YAML, not JSON)
- [ ] #35: Implement CAMI_WORKSPACE_PATH environment variable support
- [ ] #34: Standardize manifest format (capabilities vs deployed)
...
- [ ] All skills reference `config.yaml` (not `config.json`)
- [ ] Schema matches existing CAMI format: `agent_sources`, `deploy_locations`
- [ ] All skills check `$CAMI_WORKSPACE_PATH` before using default
- [ ] Single manifest format documented and used across all skills
- [ ] `reference/config-schema.md` documents the ACTUAL existing format
- [ ] Existing CAMI workspaces work without migration
